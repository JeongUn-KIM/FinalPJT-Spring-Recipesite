...
<body>
	...
	<c:forEach items="${userlist }" var="userlist" varStatus="status">
		<tr>
			...
			<td><button type='button' id="${userlist.user_no}${userlist.user_phone}">수정하기</button></td>
			<td><button type='button' id="${userlist.user_no}">삭제</button></td>
		</tr>
			<div class="modal" id="${userlist.user_no}${status.index}"> 
				<div class="modal-con" id="${userlist.user_phone}"> 
					<h3>회원정보</h3>
					...
					<br>
					<button type='button' id="${userlist.user_phone}${status.index}" onclick="test3${userlist.user_no}();">완료</button>
			...
			<script>	
				...
				$(document).ready(function(){
					$("#${userlist.user_no}${userlist.user_phone}").on("click", function(){
						$("#${userlist.user_no}${status.index}").fadeIn(); 
					});
					$(".modal_cancel").on("click", function(){
						$("#${userlist.user_no}${status.index}").fadeOut(); 
					});
				});	
				
			
				$(document).ready(function(){
					$("#${userlist.user_no}").on("click", function(){
...
<%-- $(document).ready(function(){
					 $("#${userlist.user_phone}${status.index}").on("click", function(){
						$(location).attr("href", "http://localhost:9009/usermodify?user_no=" + 
								${userlist.user_no} + "&user_email=" + 
								$('#${userlist.user_no}user_email').val() + "&user_pw=" + 
								$('#${userlist.user_no}user_pw').val() + "&user_name=" + 
								$('#${userlist.user_no}user_name').val() + "&user_phone=" + 
								$('#${userlist.user_no}user_phone').val() + "&user_address=" + 
								$('#${userlist.user_no}user_address').val());
					}); 
				});  --%>
